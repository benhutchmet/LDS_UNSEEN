...
# ==============================================================================
# Ben notes on Lainey's updates
# ==============================================================================
# * Lainey model ramps up nuclear generation given deficit
# * Lainey model ramps down nuclear generation when store is drawn down 
#     to meet demand
# Notes on nuclear generation from diss:
# * Initialisation: All days across the historical period are initially set to the
#     minimum generation capacity factor of 0.6. This is then scaled multiplicatively
#     by the installed capacity specified by the CCC for each scenario.
# * Annual Generation Correction: Each year’s annual nuclear generation is
#     calculated. A scaling factor is applied to normalise the nuclear output to match
#     the annual generation targets set by the CCC.
# * Daily Generation Correction: The nuclear output is adjusted to ensure it stays
#     within the daily minimum and maximum limits (set by the capacity factors and
#     installed capacity).
# * Iterative Adjustment:: If the annual target is not met after the initial
#     adjustment, an iterative adjustment is performed. The necessary net
#     adjustment is calculated as the difference between the target and the current
#     total generation. The nuclear output is then preferentially adjusted, adding
#     equally to days in deficit first. If all deficits are addressed, any remaining
#     adjustment is distributed across all days. This step is repeated until the annual
#     target is met within 0.1 TWh.
